...
/*
  ...
  If is an increment and the comparator is < or <=, the condition is in the form
  var <comparator> B and the minimum value is the variable init.
  If is an increment and the comparator is > or >=, the condition is in the form
  B <comparator> var and the minimum value is the variable init.
  If is an decrement and the comparator is < or <=, the condition is in the form
  B <comparator> var and the minimum value is the B+1 value for the < comparator
  and B value for the <= comparator.
  If is an decrement and the comparator is > or >=, the condition is in the form
  var <comparator> B and the minimum value is the B+1 value for the > comparator
  and B value for the >= comparator.

  To generate the correct indexes for the Kernel, we need some transformations:
  for(var=A; var [<, <=, >, >=] B; var[++, --, +=C, -=C, =var+C, =var-C]) {
    C = 1 for ++ and --
  }
...
///
unsigned CodeGenFunction::GetNumNestedLoops(const OMPParallelForDirective &S) {
  unsigned nLoops = 0;
...
void CodeGenFunction::EmitOMPTargetDataDirective(const OMPTargetDataDirective &S) {
  ...
  if (CGM.getLangOpts().MPtoGPU) {
    ...
    EmitStmt(CS->getCapturedStmt());
    EmitSyncMapClauses(OMP_TGT_MAPTYPE_FROM); // Fixed. Dominance (target-data-6c)
