...
void CodeGenFunction::EmitOMPParallelForDirective(
  ...
  if (CGM.getLangOpts().MPtoGPU) {
    ...
    // If is an decrement and the comparator is < or <=, the condition is in the form B <comparator> var and the minimum value is the B+1 value
    //   for the < comparator and B value for the <= comparator.
    // If is an decrement and the comparator is > or >=, the condition is in the form var <comparator> B and the minimum value is the B+1 value
    //   for the > comparator and B value for the >= comparator.
