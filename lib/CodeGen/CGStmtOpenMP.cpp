...
void CodeGenFunction::EmitOMPParallelForDirective(
  ...
  if (CGM.getLangOpts().MPtoGPU) {
    ...
    if (verbose) llvm::errs() << ">>> (parallel for) Emit cl_create_kernel\n";
	...
	//Prototype checkers to correctly generate the For loop in the Kernel function

	bool compareEqual, isLesser, isIncrement;
	llvm::Value *A, *B, *C;
	...
	if (verbose) llvm::errs() << ">>>Adding induction var " << *IVal << " in LocalVars\n";
	...
	if(isa<CompoundAssignOperator>(inc)) {	// i(+/-)=incr
		...
		UnaryOperator *BO = dyn_cast<UnaryOperator>(inc);
	...
	// Restriction for the Canonical Loop Form (Lines 19-26, Page 60, Document http://www.openmp.org/mp-documents/OpenMP4.0.0.pdf)
	// If is an increment and the comparator is < or <=, the condition is in the form var <comparator> B and the minimum value is the variable init.
	// If is an increment and the comparator is > or >=, the condition is in the form B <comparator> var and the minimum value is the variable init.
	// If is an decrement and the comparator is < or <=, the condition is in the form B <comparator> var and the minimum value is the B+1 value for the < comparator and B value for the <= comparator.
	// If is an decrement and the comparator is > or >=, the condition is in the form var <comparator> B and the minimum value is the B+1 value for the > comparator and B value for the >= comparator.


	// To generate the correct indexes for the Kernel, we need some transformations:

	// for(var=A; var [<, <=, >, >=] B; var[++, --, +=C, -=C, =var+C, =var-C]) {C = 1 for ++ and --}

	// The Kernel generated by this loop is:

	// if(var < K) {
	//	All uses of (var) are replaced by (var*C + min)
	//}

	// Where:

	// K = abs(B-A)/C when comparator is > or <
	// K = abs(B-A+1)/C when comparator is >= or <=

	// min = A when is an increment
	// min = B when is an decrement and the operator is >= or <=
	// min = B+1 when is an decrement and the operator is > or <
...
//	llvm::Value *IV = dyn_cast<llvm::Instruction>(IVal)->getOperand(0);
...
//    std::string CName = (D->getDecl())->getNameAsString();
	std::string CName = "_UB_";
//    llvm::Type *ITy = getVarType (IV);
//    llvm::Type *CTy = getVarType (CV);
//	llvm::Type *CTy = getVarType(B);

    llvm::Type *ITy = CGM.IntPtrTy;
    ...
    if (verbose) llvm::errs() << ">>> &Condition Variable= " << *CVRef << "\n";
    ...
    if (verbose) llvm::errs() << ">>> &Minimum Variable= " << *CVRef2 << "\n";
    ...
    if (verbose) llvm::errs() << ">>> &Increment Variable= " << *CVRef3 << "\n";
    ...
    CLOS << ") {\n   ";
    CLOS << "int _ID_ = get_global_id(0);\n   ";
    CLOS << "int " << IName << " = _INC_*_ID_ + _MIN_;\n   ";
    ...
    llvm::Value *WorkSize[] = {Builder.CreateIntCast(nCores, CGM.Int64Ty, false)};
