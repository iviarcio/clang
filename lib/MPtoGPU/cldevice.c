...
void _cldevice_init(int rtlmode) {
    ...
    if (_device == NULL) {
        ...
        _status = clGetPlatformInfo(_platform, CL_PLATFORM_EXTENSIONS, sizeof(extension_string), extension_string,
                                    NULL);
...
void _set_default_device(cl_uint id) {
    ...
    status = clGetDeviceInfo(_device[_clid], CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS, sizeof(cl_uint), &maxWIDimensions,
                             NULL);
    if (status != CL_SUCCESS) {
